/*
 * Script Name: Cancel Snipe Helper
 * Version: v1.0.3
...
var scriptConfig = {
    scriptData: {
        ...
        version: 'v1.0.3', // FIX #1: war v1.0.2, muss mit Header übereinstimmen
        author: 'RedAlert',
...
window.twSDK = {
    ...
    getBuildingsInfo: async function () {
        ...
        const LAST_UPDATED_TIME = localStorage.getItem('buildings_info_last_updated');
        let buildingsInfo = [];

        // FIX #3: else-Branch entfernt (war durch ?? 0 nie erreichbar) und
        //         unitInfo -> buildingsInfo korrigiert
    ...
    getContinentByCoord: function (coord) {
        // FIX #7: i >= x -> i <= x und j >= y -> j <= y (Vergleichsoperatoren waren invertiert)
    ...
    getWorldConfig: async function () {
        ...
        const LAST_UPDATED_TIME = localStorage.getItem('world_config_last_updated');
        let worldConfig = [];

        // FIX #4: else-Branch entfernt (war durch ?? 0 nie erreichbar) und
        //         unitInfo -> worldConfig korrigiert
        if (LAST_UPDATED_TIME !== null) {
            if (Date.parse(new Date()) >= parseInt(LAST_UPDATED_TIME) + TIME_INTERVAL) {
                const response = await jQuery.ajax({
            ...
            try {
                ...
                switch (entity) {
                    ...
                    case 'ally':
                        ...
                        break;
                    case 'conquer':
                        // FIX #5: zweites newPlayerId -> oldPlayerId
                        responseData = data
                            .filter((item) => {
                                if (item[0] != '') {
                                    return item;
...
(async function () {
    ...
    function handleCalculateSnipe() {
        jQuery('#raCalculateCancelSnipeBtn').on('click', function (e) {
            ...
            // FIX #6: return nach Validierungsfehler fehlte
